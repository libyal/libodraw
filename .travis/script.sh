...
then
	docker run -t -v "${PWD}:/libodraw" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libodraw && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libodraw;
