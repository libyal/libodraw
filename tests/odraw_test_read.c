...
/* Define to make odraw_test_read generate verbose output
...
/* Tests libodraw_handle_read_buffer
 * Returns 1 if successful, 0 if not or -1 on error
 */
int odraw_test_read_buffer(
     libodraw_handle_t *handle,
     size64_t input_size,
     size64_t expected_size )
{
	...
	{
		read_size = ODRAW_TEST_READ_BUFFER_SIZE;
...
/* Tests libodraw_handle_read_buffer_at_offset
 * Returns 1 if successful, 0 if not or -1 on error
 */
int odraw_test_read_buffer_at_offset(
     libodraw_handle_t *handle,
     off64_t input_offset,
     size64_t input_size,
     off64_t expected_offset,
     size64_t expected_size )
{
	...
	while( remaining_size > 0 )
	{
		...
		read_count = libodraw_handle_read_buffer_at_offset(
			      handle,
			      buffer,
			      read_size,
			      input_offset,
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int odraw_test_seek_offset_and_read_buffer(
     libodraw_handle_t *handle,
     off64_t input_offset,
     int input_whence,
     size64_t input_size,
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int odraw_test_read_from_handle(
     libodraw_handle_t *handle,
     size64_t media_size )
{
	int result = 0;
	 ...
	 */
	result = odraw_test_seek_offset_and_read_buffer(
	          handle,
	          0,
	          SEEK_SET,
	          media_size,
	 ...
	 */
	result = odraw_test_read_buffer_at_offset(
	          handle,
	          (off64_t) ( media_size / 7 ),
	          media_size / 2,
...
#endif
{
	...
	if( odraw_test_read_from_handle(
	     handle,
	     media_size ) != 1 )
	{
		fprintf(
